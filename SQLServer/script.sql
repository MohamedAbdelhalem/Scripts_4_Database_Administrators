...
ALTER TABLE dbo.F_BAB_L_M001 ADD UPLOAD_DATE AS dbo.F_BAB_L_M001_(XMLRECORD) PERSISTED;
CREATE NONCLUSTERED INDEX UPLOAD_DATE_BAB_L_M001_ ON dbo.F_BAB_L_M001(UPLOAD_DATE ASC);
ALTER TABLE dbo.F_BAB_L_M001 ADD FILE_DATE AS dbo.F_BAB_L_M001_(XMLRECORD) PERSISTED;
...
select 
substring(syntax,1, charindex(' ', syntax) -1)+'/*       '+
'%'+cast(round(cast(pct_id * 100.0 / count(*) over() as float),2) as varchar(10))+replicate(' ',10 - len(cast(round(cast(pct_id * 100.0 / count(*) over() as float),2) as varchar(10))))+'*/  ' + 
substring(syntax,charindex(' ', syntax)+1,len(syntax)) syntax, command, sub_command, 
table_name, 
is_table_exist, 
column_name, 
is_column_exist, 
...
from (
select 
row_number() over(order by table_name) pct_id, 
--row_number() over(order by command) pct_id, 
syntax, command, sub_command, 
table_name, 
is_table_exist, 
column_name, 
is_column_exist, 
...
case when sub_command = 'Add Column' or command = 'Create Index' then case when (select count(*) from sys.columns where name = column_name collate Arabic_100_CI_AS and object_id = object_id(table_name collate Arabic_100_CI_AS)) > 0 then 1 else 0 end 
else '' end is_column_exist,
case when sub_command in ('Add Column') or command in ('Create Index','Drop Index') then case when (select count(*) from sys.tables where object_id = object_id(table_name collate Arabic_100_CI_AS)) > 0 then 1 else 0 end 
else '' end is_table_exist,
case when sub_command = 'Add Column' then case when (select count(*) from sys.objects where object_id = object_id(fn_name collate Arabic_100_CI_AS) and type = 'fn') > 0 then 1 else 0 end 
...
from master.[dbo].[Separator](@syntax,';'))a)b)c)d
where table_name != 'dbo.FBNK_FUNDS_TRANSFER#HIS'
and is_index_exist = 0
and is_table_exist = 1 and is_column_exist = 1
