USE [MineSweeper]
GO
/****** Object:  StoredProcedure [dbo].[sp_minesweeper_debug]    Script Date: 9/22/2022 12:24:22 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[sp_minesweeper_debug]
(@Mine int = 120, @Width int = 30, @Height int = 16)
as
begin
...
declare @big_loop int = 0, @square varchar(10), @row varchar(max)
set @loop = 0
while @big_loop < @Width * @Height
begin
set @loop = @loop + 1

select @square = case _mine when '.' then _mine else _count end --Mine
from (
...
where cast(number as int) = @big_loop + 1 

set @row = isnull(@row+char(9),'')+@square
